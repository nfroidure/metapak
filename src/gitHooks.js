...
function buildPackageGitHooks({
  ...
  .then((hooksBuilders) => {
    hooksBuilders = hooksBuilders
    .reduce(
      (hooks, hooksBuilder) =>
        hooksBuilder(hooks, packageConf),
      {}
    );
    return hooksBuilders;
  })
  .then(hooks =>
    Promise.all(Object.keys(hooks).map((hookName) => {
      const hookContent = '#!/bin/sh' + os.EOL +
      ...
      return fs.readFileAsync(hookPath, 'utf-8')
      .catch((err) => {
